...
impl Transform for AddNullable {
    fn transform(&mut self, schema: &mut Schema) {
        if schema.has_type("null") {
            ...
            if only_allows_null {
                // For null-only schemas, keep {"type": "null"} for JSON Schema compatibility
                // Don't add const or remove type - Gemini and other validators need the type field
                // Even in OpenAPI 3.0, nullable should be used with a type field
